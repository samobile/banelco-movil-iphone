...
//		}
    ...
    //todo bien?
    //Consulto WS consultarConfiguracionBanco para traer parametros de configuracion
    WS_ConsultarConfiguracionBanco *req = [[WS_ConsultarConfiguracionBanco alloc] init];
    req.userToken = [context getToken];
    req.json = [NSString stringWithFormat:@"{\"banco\":\"%@\"}",context.banco.idBanco];
    context.extraccionEnabled = NO;
    id resp = [WSUtil execute:req];
    if (!resp){
        [CommonUIFunctions showAlert:@"Ingreso" withMessage:@"En este momento no se puede realizar la operación. Reintenta más tarde." andCancelButton:@"Cerrar"];
    }
    else if ([resp isKindOfClass:[NSError class]]) {
        NSString *errorDesc = [[(NSError *)resp userInfo] valueForKey:@"description"];
        [CommonUIFunctions showAlert:@"Ingreso" withMessage:errorDesc cancelButton:@"Cerrar" andDelegate:nil];
    }
    else if ([[(NSString *)resp lowercaseString] isEqualToString:@"true"]) {
        context.extraccionEnabled = YES;
    }
    //consultarConfiguracionBanco
