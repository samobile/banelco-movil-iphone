...
@implementation CreditCardMenuMovimientosVC
...
- (void)accionWithDelegate:(WheelAnimationController *)delegate {
	NSLog(@"accion en CreditCardMenuMovimientosVC");
	id result;
	...
	if ([result isKindOfClass:[NSError class]]) {
		...
		if ([tarjetas count] == 0) {
			[CommonUIFunctions showAlert:@"Consulta Movimientos" withMessage:@"No hay movimientos." cancelButton:@"Volver" andDelegate:self];
		}
		else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (tarjetas != nil && indexPath.row <= [tarjetas count]) {
		...
		CreditCardMovimientosVC *vc = [[CreditCardMovimientosVC alloc] initWithNumeroTarjeta:aCreditCard.numero];
